...
Strategy.prototype.authenticate = function (req) {
    ...
    function verify(err, token) {
        ...
        if (that._passReqToCallback) {
            that._verify(req, token, verified, refreshToken, expiresIn, rawToken);
        } else {
            that._verify(token, verified, refreshToken, expiresIn, rawToken);
        }
    }

    this._getRawToken(req, function (rawTkn, refrTkn, expIn) {
        rawToken = rawTkn;
        if (!rawToken) {
